<?php
/**
 * Add Admin Page - Self-contained version
 */
        ...
        if ($csrf_token === ($_SESSION['csrf_token'] ?? '')) {
            ...
            if (empty($username) || empty($password) || empty($full_name)) {
                ...
                if ($stmt->fetchColumn() > 0) {
                    ...
                    $stmt = $db->prepare("INSERT INTO admin_users (username, password, full_name, email, role, status) VALUES (?, ?, ?, ?, ?, ?)");
                    $stmt->execute([$username, $password_hash, $full_name, $email, $role, $status]);
